...
class CartScreen extends StatelessWidget {
  ...
  void _launchWhatsApp(BuildContext context, CartModel cart) async {
    ...
    String numeroWhatsapp =
        "+5547992680847"; // INSIRA AQUI O NÚMERO DE WHATSAPP DA LOJA

    String message = "Olá! Gostaria de fazer um pedido:\n\n";
    for (var item in cart.items) {
      message +=
          "${item.quantity}x ${item.product.name} - ${formatCurrency.format(item.product.price * item.quantity)}\n";
    }
    message += "\n*Total: ${formatCurrency.format(cart.totalPrice)}*";

    // Codifica a mensagem para URL
    final Uri whatsappUrl = Uri.parse(
      "https://wa.me/message/TH6UCYQCCKX7D1$numeroWhatsapp?text=${Uri.encodeComponent(message)}",
    );
                  ...
                  child: ListTile(
                    leading: CircleAvatar(
                      ...
                      onBackgroundImageError:
                          ...
                          (_, __) {}, // Evita erro se a imagem não carregar
                    ),
                    ...
                    trailing: Row(
                      ...
                      children: [
                        IconButton(
                          ...
                          onPressed: () => cart.decrement(cartItem),
                        ),
                        Text('${cartItem.quantity}'),
                        IconButton(
                          icon: const Icon(Icons.add),
                          onPressed: () => cart.increment(cartItem),
                        ),
                      ],
                    ),
                  ),
